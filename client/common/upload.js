...
* @Last Modified time: 2018-01-18 15:40:03
*/
...
import {getDevice} from './util';
  ...
  Class.prototype.upload = function(files, type){
    ...
    ,iframeSend = function(){
      ...
      var endTime = startTime + 600000*3;
      //获取响应信息
    ...
    switch(options.accept){
      case 'mix':// pdf + 图片
        if(!RegExp('\\w\\.('+ (exts || 'pdf|png|jpeg|jpg|gif|doc|docx|ppt|pptx') +')$', 'i').test(escape(value))){
          toast.fail('选择的文件中包含不支持的格式');
          return elemFile.value = '';
        }
      break;
      case 'file': //一般文件
        if(!RegExp('\\w\\.('+ (exts || 'pdf|doc|docx') +')$', 'i').test(escape(value))){
          toast.fail('选择的文件中包含不支持的格式');
          return elemFile.value = '';
        }
      break;
      case 'video': //视频文件
        if(!RegExp('\\w\\.('+ (exts || 'mp4|mov|avchd|mpeg|avi|wmv') +')$', 'i').test(escape(value))){
          toast.fail('选择的视频中包含不支持的格式');
          return elemFile.value = '';
        }
      break;
      case 'audio': //音频文件
        if(!RegExp('\\w\\.('+ (exts || 'mp3|wav|mid') +')$', 'i').test(escape(value))){
          toast.fail('选择的音频中包含不支持的格式');
          return elemFile.value = '';
        }
      break;
      default: //图片文件
        layui.each(value, function(i, item){
          if(!RegExp('\\w\\.('+ (exts || 'jpg|png|gif|jpeg$') +')', 'i').test(escape(item))){
            check = true;
          }
        });
        if(check){
          toast.fail('选择的图片中包含不支持的格式');
          return elemFile.value = '';
        }
      break;
    }
    ...
    if(options.number && that.fileLength > options.number){
      return toast.fail('同时最多只能上传的数量为：'+ options.number);
    }
    ...
    if(options.size > 0 && !(device.ie && device.ie < 10)){
      ...
      if(limitSize) return toast.fail('文件不能超过'+ limitSize);
    }
    send();
  };
    ...
    options.elem.off('upload.start').on('upload.start', function(){
      ...
      if(data){
        try{
          ...
          console.error('Upload element property lay-data configuration item has a syntax error: ' + data)
        }
      }
